...
(function (root) {
  ...
  function uniqueListId(settings, name) {
    ...
    var id = base;
    var n = 2;
    while (LL.settings.getList(settings, id)) id = base + "-" + n++;
    return id;
  }
  ...
  function cleanRules(raw) {
    return LL.rules.normalizeAll(raw).filter(function (r) { return r.from.trim() !== ""; });
  }
  ...
  /* Reads, in order:
       { lingoloco, lists: [ { name, rules } ] }            a bundle
       ...and the same shape from Buzz Kill for Firefox, whose marker key
       differs but whose structure is identical, so lists move between the
  ...
  function merge(existing, incoming) {
    ...
    incoming.forEach(function (rule) {
      ...
      if (!hit) {
        var copy = Object.assign({}, rule, { id: LL.rules.newId(), to: rule.to.slice() });
        index.set(key, copy);
        merged.push(copy);
        added++;
        return;
