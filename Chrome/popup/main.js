...
(function (root) {
  ...
  Promise.all([LL.settings.load(), currentTab()]).then(function (out) {
    pop.set(out[0]);
    LL.theme.apply(out[0].theme);

    var tab = out[1];
    if (tab) {
      pop.page.tabId = tab.id;
      pop.page.host = LL.sites.hostOf(tab.url || "");
    }

    Object.keys(LL.popPanels).forEach(function (name) { LL.popPanels[name].wire(); });
    wireFooter();
