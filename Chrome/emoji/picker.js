...
(function (root) {
  ...
  function render() {
    ...
    if (query) {
      var hits = LL.emojiSearch(data, query);
      statusEl.textContent = hits.length
        ? hits.length + (hits.length === 1 ? " match" : " matches")
        : "Nothing matches \u201c" + query + "\u201d";
      hits.slice(0, LIMIT).forEach(function (row) {
        frag.appendChild(cell(data.chars[row], LL.emojiData.nameOf(row)));
      });
    } else {
      ...
      for (var i = 0; i < data.count; i++) {
        if (data.groupOf[i] === group) frag.appendChild(cell(data.chars[i], LL.emojiData.nameOf(i)));
      }
    }
  ...
  function open(anchor, options) {
    ...
    LL.emojiData.load().then(function () {
      data = LL.emojiData.current();
  ...
  LL.emojiPicker = { open: open, close: close };
