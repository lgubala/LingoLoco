...
(function (root) {
  ...
  function render() {
    ...
    var live = LL.schedule.resolveListId(settings, new Date());
    ...
    if (settings.schedule.enabled) {
      var liveList = LL.settings.getList(settings, live);
      note.hidden = false;
  ...
  function wire() {
    app.el("list-new").addEventListener("click", function () {
      ...
      var list = { id: LL.share.uniqueListId(settings, name.trim()), name: name.trim(), rules: [] };
    ...
    app.el("list-delete").addEventListener("click", function () {
      ...
      app.set(LL.settings.normalize(settings));
      app.save(true);
      app.render();
    });
  }
