...
(function (root) {
  ...
  function render() {
    ...
    var inside = LL.settings.getList(settings, s.inside);
    var outside = LL.settings.getList(settings, s.outside);
    // The header says what's happening while the panel is shut.
    app.el("sched-state").textContent = !s.enabled
      ? "Off"
      : (LL.schedule.inWindow(s, new Date())
