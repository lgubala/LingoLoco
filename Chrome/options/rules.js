...
(function (root) {
  ...
  function ruleRow(list, rule) {
    ...
    var toInput = LL.fields.text(rule.to.join(", "), "your word, or two", function (v) {
      rule.to = LL.fields.parseVariants(v);
      app.save();
      app.render("proof");
    });
    toInput.title = "Comma-separated: they take turns down the page";
    toWrap.append(toInput, LL.fields.emojiButton(toInput));
    ...
    row.append(
      sw,
      LL.fields.text(rule.from, "word on the page", function (v) {
        rule.from = v;
        app.save();
        app.render("proof");
      }),
      arrow,
      toWrap,
      LL.fields.check(!!rule.matchCase, "Match capitalisation exactly", function (v) {
        rule.matchCase = v;
        app.save();
        app.render("proof");
      }),
      LL.fields.check(rule.wholeWord !== false, "Match whole words only", function (v) {
        rule.wholeWord = v;
        app.save();
        app.render("proof");
      }),
  ...
  function add() {
    ...
    var to = LL.fields.parseVariants(toEl.value);
    var list = app.currentList();
    var existing = null;
    ...
    if (existing) {
      ...
      list.rules.push(LL.rules.normalize({ from: from, to: to }));
    }
  ...
  function wire() {
    ...
    app.el("to").parentNode.appendChild(LL.fields.emojiButton(app.el("to")));
  }

  app.register("rules", render);
  LL.panels = LL.panels || {};
  LL.panels.rules = { wire: wire };
