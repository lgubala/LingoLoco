...
(function (root) {
  ...
  var IDLE_NOTE = "Exports are plain JSON. Lists from Buzz Kill for Firefox work too.";
  var noteTimer = null;
  ...
  function exportText() {
    return JSON.stringify(LL.share.exportLists(selectedLists()), null, 2);
  }

  function exportFilename() {
    var chosen = selectedLists();
    if (chosen.length === 1) return "lingoloco-" + LL.share.slug(chosen[0].name) + ".json";
    return "lingoloco-" + chosen.length + "-lists.json";
  }
  ...
  function fateOf(list) {
    ...
    if (mode === "name") {
      return LL.share.findByName(app.get(), list.name) ? "merge" : "new list";
    }
    return "new list";
  }
  ...
  function stage(text, fallbackName) {
    var parsed = LL.share.parse(text);
  ...
  function applyStaged() {
    ...
    lists.forEach(function (incoming) {
      ...
      else if (mode === "name") target = LL.share.findByName(settings, incoming.name);

      if (target) {
        var result = LL.share.merge(target.rules, incoming.rules);
        target.rules = result.rules;
        added += result.added;
        extended += result.extended;
        mergedInto++;
      } else {
        var name = LL.share.uniqueListName(settings, incoming.name);
        var list = {
          id: LL.share.uniqueListId(settings, name),
          name: name,
          rules: incoming.rules.map(function (r) { return LL.rules.normalize(r); })
        };
