...
(function (root) {
  ...
  function normalize(raw) {
    ...
    var lists = Array.isArray(raw.lists) ? raw.lists.map(function (l) {
      return {
        id: typeof l.id === "string" && l.id ? l.id : BK.rules.newId("l"),
        name: typeof l.name === "string" && l.name.trim() ? l.name.trim() : "Untitled",
        rules: BK.rules.normalizeAll(l.rules)
      };
    }) : [];
    ...
    if (!lists.length) {
      ...
      if (Array.isArray(raw.rules) && raw.rules.length) {
        lists[0].rules = BK.rules.normalizeAll(raw.rules);
      }
    }
  ...
  /* Lists outgrow storage.sync's 8 KB per-item cap, so state lives in
     storage.local. Version 1.0 used sync; those rules are pulled across once. */
  function load() {
    return new Promise(function (resolve) {
      api().storage.local.get(null, function (stored) {
        if (stored && stored.lists) { resolve(normalize(stored)); return; }
        try {
          api().storage.sync.get(null, function (old) {
            resolve(normalize(stored && Object.keys(stored).length ? stored : old));
          });
        } catch (e) {
